...
const result = await generateText({
  ...
  prompt: `Use wallet ${walletId}. Submit a pact first. Then transfer 0.001 SETH to ${destination} on SETH. After that attempt 0.005 SETH, and if denied retry with a compliant amount.`,
  tools: {
    submit_pact: tool({
    ...
    transfer_tokens: tool({
      ...
      execute: async ({ wallet_uuid, dst_addr, token_id, amount, request_id, pact_id }) => {
